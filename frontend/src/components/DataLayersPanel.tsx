...
import { RadioTower, CloudSun, Anchor, Truck, AlertTriangle, Package, ShoppingCart, LogOut, Zap, Cloud, Ship, FileWarning, Users } from "lucide-react";
  ...
  return (
    <div className="h-full w-[300px] glass-panel p-4 flex flex-col gap-6 z-10">
      <div className="flex flex-col gap-1">
        ...
        {/* Role badge */}
        <div className={`flex items-center gap-1.5 px-1 py-0.5 w-fit border ${isSeller ? "border-cyan-900/40 bg-cyan-950/10" : "border-purple-900/40 bg-purple-950/10"}`}>
          {isSeller
            ? <Package className="w-2.5 h-2.5 text-cyan-600" />
            : <ShoppingCart className="w-2.5 h-2.5 text-purple-500" />
          }
          <span className={`text-[8px] uppercase tracking-widest font-bold ${isSeller ? "text-cyan-600" : "text-purple-500"}`}>
            {fullName || (isSeller ? "Seller" : "Buyer")}
          </span>
        </div>
      ...
      <div className="mt-auto pt-6 flex flex-col gap-4">
        <div className={`p-3 border flex flex-col gap-2 ${isSeller ? "border-cyan-900/30 bg-cyan-950/10" : "border-purple-900/30 bg-purple-950/10"}`}>
          <div className="flex items-center gap-2">
            {isSeller
              ? <Package className="w-3 h-3 text-cyan-500" />
              : <ShoppingCart className="w-3 h-3 text-purple-400" />
            }
            <span className={`text-[10px] uppercase tracking-widest font-bold ${isSeller ? "text-cyan-500" : "text-purple-400"}`}>
              {isSeller ? "Send Product" : "Receive Product"}
            </span>
          </div>
          <div className={`text-[8px] uppercase tracking-wider mb-1 ${isSeller ? "text-zinc-600" : "text-zinc-600"}`}>
            {isSeller ? "Schedule outbound shipment" : "Place inbound order"}
          </div>
          ...
          >
            {isSeller ? "Dispatch Shipment →" : "Place Order (Receive)"}
          </button>
        </div>
        ...
        <div className="p-3 border border-amber-900/30 bg-amber-950/10 flex flex-col gap-2">
            ...
            {/* Signal type buttons — visible to both seller and buyer */}
            <div className="grid grid-cols-2 gap-1.5">
              {/* Weather / Cyclone */}
              <button
                onClick={() => injectDemoSignal("weather")}
              ...
              {/* Port Congestion */}
              <button
                onClick={() => injectDemoSignal("port_congestion")}
                disabled={isInjecting}
              ...
              {/* Road Closure */}
              <button
                onClick={() => injectDemoSignal("road_closure")}
                disabled={isInjecting}
              ...
              {/* Customs Delay */}
              <button
                onClick={() => injectDemoSignal("customs_delay")}
                disabled={isInjecting}
              ...
              {/* Civil Unrest */}
              <button
                onClick={() => injectDemoSignal("civil_unrest")}
                disabled={isInjecting}
